    ...
    async def cat(self, ctx):
        ...
        async with self.bot.session.get("https://aws.random.cat/meow") as r:
            if r.status == 200:
                ...
                await ctx.send(
                    f"random.cat didn't send an image. try again later. (http {r.status}"
                )
    ...
    async def dog(self, ctx):
        async with self.bot.session.get(self.decide_source()) as r:
            if r.status == 200:
                ...
                await ctx.send(
                    f"{r.host} didn't send back anything. try again later? (http {r.status})"
                )
    ...
    async def lizard(self, ctx):
        ...
        async with self.bot.session.get("https://nekos.life/api/lizard") as r:
            if r.status == 200:
                ...
                await ctx.send(
                    f"nekos.life didn't send anything back. try again later? (http {r.status})"
                )
